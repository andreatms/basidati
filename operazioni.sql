...
RETURNS TABLE(CodiceSpedizione INT, NominativoMittente VARCHAR(20) NominativoDestinatario VARCHAR(20), IndirizzoDestinazione VARCHAR(30) CivicoDestinazione INT, Stato VARCHAR(20), Costo INT)
$BODY$
SELECT CodiceSpedizione, NominativoMittente, NominativoDestinatario, IndirizzoDestinazione, CivicoDestinazione, Stato, Costo
FROM Spedizione
...
RETURNS TABLE(Matricola INT, Nome VARCHAR(20), Cognome VARCHAR(20), Ruolo VARCHAR(20), Sezione VARCHAR(10), Stipendio INT)
$BODY$
...
RETURNS TABLE(Matricola INT, Nome VARCHAR(20), Cognome VARCHAR(20), Ruolo VARCHAR(20), Filiale INT)
$BODY$
SELECT Dipendente.Matricola, Dipendente.Nome, Dipendente.Cognome, Ruolo.Nome, Filiale.CodiceFiliale
FROM Dipendente JOIN Afferenza ON Dipendente.Matricola = Afferenza.Dipendente
                JOIN Filiale ON Afferenza.Filiale = Filiale.CodiceFiliale
                JOIN Impiego ON Dipendente.Matricola = Impiego.Dipendente
                JOIN Ruolo ON Impiego.Ruolo = Ruolo.Sigla
